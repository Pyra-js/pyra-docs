...
The CLI is provided by the `@pyra-js/cli` package, which exposes the `pyra` binary. The fastest way to get started is by scaffolding a new project, this installs the CLI as a dev dependency automatically:
...
In team settings, keeping `@pyra-js/cli` as a dev dependency and using `package.json` scripts ensures everyone runs the same version:
